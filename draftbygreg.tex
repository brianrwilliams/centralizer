...
See section~\ref{sec: enveloping} for a summary of the enveloping $E_n$ algebra construction, and see section~\ref{sec: centralizer} 
for a summary of centralizers of maps of $E_n$ algebras.

\begin{thm}
\label{thm: centralizer}
Let $\UU_n \fg$ be the enveloping $E_n$ algebra of a dg Lie algebra $\fg$ and $f: \UU_n \fg \to B$ a map of augmented $E_n$ algebras which gives $B$
the structure of an $E_n$ algebra in $\fg$-modules\footnote{see Proposition~\ref{P:inducedEngModStruct}}. 
Then, the centralizer ${\frak Z}_{E_n}(f)$ is the $E_n$-algebra given by the Chevalley-Eilenberg 
algebra $\clies(\fg, B)$ with the convolution $E_n$-product.
\end{thm}

To any $\fg$-module $M$ (wether $\fg$ is a dg Lie algebra or an $L_\infty$-algebra), we can associate its Chevalley-Eilenberg cochain complex defined 
by 
\begin{equation}
