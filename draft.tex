...
\]
and that the summand $k$ encodes the defining map~$f: \fg \rightsquigarrow \fh$.
(Abusively, here $f$, $\fg$, and $\fh$ denote generic elements and not those appearing in the theorem we are proving.)
...
\]
where $\iota_W$ denotes the inclusion $\Omega^*_c(W) \hookrightarrow \Omega^*(W)$ extended in the obvious way to this setting.
\end{dfn}
...
Given an $E_n$-algebra $A$ the pair $(\fZ_n (A) , A)$ is an algebra over the $(n+1)$-dimensional Swiss cheese operad. 
The first objective is to define a certain ``classical limit" of this pair in the case that $A$ is an $E_n$-enveloping algebra. 
...
The associated graded pair in Proposition \ref{prp:swiss1} carries the additional Poisson-type structure. 
...
A constructible factorization algebra $\bA$ on $M \times \RR_{\geq 0}$ determines a pair of factorization algebras $(\pi_* \bA , \bA_{\partial})$ on $M$ which locally define the $E_1$-model for $(n+1)$-Swiss cheese algebras. 

The next goal of this section is to provide a model of the classical limit of the $E_n$-center as a constructible factorization algebra. 
...
We accomplish Proposition \ref{prp:swiss1} by exhibiting a filtration at the level of strict factorization algebras in our model of the $E_n$-center. 
...
\begin{prp}
The pair $(\bZ_n (\fg), \bU_n(\fg)$ is a filtered, \brian{not sure what we're calling this}. 
Further, the associated graded $({\rm Gr} \; \bZ_n (\fg), {\rm Gr} \; \bU_n(\fg)$ is a $\PP_0$ \brian{not sure what we're calling this}. 
\end{prp}
