...
\item  In particular, given an $E_n$ algebra $B$,  let $\tt{B}$ denote a strict prefactorization algebra modeling~$B$.
Strictification lets us associate to the map $f: \UU_n \fg \to B$, a map $\tt{f}: \tt{U}_n \fg \to \tt{B}$ of strict prefactorization algebras. 
\owen{It might be better to first use the adjunction to get a map of homotopy-coherent Lie algebras $\fg \to B$ but \dots}
\item From the map $\tt{f}$, we read off an $\fg$-action on $\tt{B}$ intertwining with the structure maps of $\tt{B}$. 
That is, we have a prefactorization algebra $\tt{B}_f$ taking values in the 1-category of chain complexes of $\fg$-representations.
\item We then construct an explicit strict prefactorization algebra $\clies(\Omega \otimes \fg, B_f)$ as a kind of ``convolution algebra.'' 
...
Fitting into such a diagram guarantees a canonical map $\clies(\Omega \otimes \fg, B_f) \to \tt{Z}_f$ as strict prefactorization algebras. 
