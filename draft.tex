...
\end{tikzcd}
\]
The first map projects onto the $\Sym^0$ component of $\clie^\bu$ and the second map uses the fact that the symmetric algebra functor is monoidal. 
...
In this section, we describe a relationship of our model of the $\EE_n$-center with a certain topological field theory. 
...
Via the obvious $n$-shifted symplectic structure on ${\rm T}^* [n] {\rm B} G$, the AKSZ formalism equips the mapping space with a $(-1)$-shifted symplectic structure. 
Classically, on manifolds without boundary, this is the complete description of topological BF theory in the Batalin--Vilkovisky formalism \cite{BV}. 
...
Moreover, the observables, is equipped with the structure of a $\PP_0$-factorization algebra.
...
In \cite{Eugene}, it is shown that the assignment $U \mapsto \clies (\cL(U))$ has the structure of a constructible $\PP_0$-factorization algebra on $M \times \RR_{\geq 0}$. 
...
\begin{proof}
Suppose $U \subset M \times \RR_{>0}$ is an open set 
\end{proof}
