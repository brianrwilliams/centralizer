...
See Lemma \ref{L:inducedFacgModstruct}. 
\item Construct an explicit strict prefactorization algebra $\clies(\Omega \otimes \fg, B_f)$ as a kind of ``convolution algebra", Lemma \ref{L:ClieFacDef}.
We then argue that it canonically fits into a centralizer diagram: there is a map of prefactorization algebras
\[
\clies(\Omega \otimes \fg, B_f) \otimes \UU_n \fg \to B_f
\]
such that precomposing with the map $1 \otimes \id_{\UU_n \fg}$ is the map~$f$, see Lemma \ref{L:ComTriangCent}.
...
\item Verify that this map is a quasi-isomorphism on disks, Lemma \ref{L:thetaquisondisk}.
...
\begin{thm}
\label{T:En=Fac} 
There is an equivalence of $\infty$-categories between $E_n$-algebras and locally constant factorization algebras.
\end{thm}
