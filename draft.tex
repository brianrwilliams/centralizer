...
\owen{Greg, please read this below.}
...
\[
{\rm Poly}_n(A) \simeq \Sym_{\widetilde{A}} (\TT_{\widetilde{A}}[1-n]),
\]
...
\begin{proof}
By our technical hypothesis, we are free to replace $A$ by a semifree resolution $(\Sym(V^\vee), \partial)$. 
...
There is a natural homotopy-locally constant sheaf of cochain complexes $\Omega^* \otimes V$ with differential $\d_{dR} + \partial$,
which is a soft replacement for the locally constant sheaf $V$.
Define the factorization algebra 
\[
F_A = (\Sym((\Omega* \otimes V)* ), \d_{dR} + \partial) 
\]
...
\owen{I'm partway done.}

3) Define the putative center as a factorization algebra by starting with the polyvector fields for this semifree resolution
...
Using our models, we obtain a generalization of the Harish--Chandra isomorphism to the $\EE_n$ center of the  enveloping $\EE_n$ algebra. 
...
\begin{prp}
There is a canonical quasi-isomorphism of $\EE_{n+1}$-algebras
\[
...
\begin{itemize}
\item if $n$ is odd, then there is a quasi-isomorphism of $\EE_{n+1}$-algebras
\[
{\frak Z}_n(\UU_n \fg) \simeq \clies (\fg) \otimes \Sym(\fh[1-n])^W.
\]
\item if $n$ is even, then there is a quasi-isomorphism of $\EE_{n+1}$-algebras
\[
...
\bZ_n(\UU_n \fg) \xto{\simeq} \clies\left(\fg, \Sym(\fg[1-n]) \right) .
\]
Since $\fg$ is semi-simple, there is a quasi-isomorphism
\[
