...
Before stating the definition of this model, we introduce the following variants of the de Rham complex of differential forms on $\RR_{\geq 0}$ and compactly supported differential forms.

Let $\Tilde{\Omega}^\bu (\RR_{\geq 0})$ be the two-term complex
\[
\{f \in  C^{\infty}(\RR_{\geq 0}) \; | \; f(0) = 0\}  \xto{\d_{\rm dR}} \Omega^1(\RR_{\geq 0})[-1]  .
\]
Notice that this is a subcomplex of the full de Rham complex on $\RR_{\geq 0}$.
In fact, this is a de Rham model for the relative cohomology of the pair $(\RR_{\geq 0}, \partial \RR_{\geq 0} = \{t = 0\})$.

Similarly, let $\Tilde{\Omega}^{\bu}_{c}(\RR_{\geq 0}) \subset \Omega^{\bu}_{c}(\RR_{\geq 0})$ be the subcomplex of compactly supported de Rham forms on $\RR_{\geq 0}$ whose zero form component vanishes at $t = 0$.

Notice that $\Tilde{\Omega}_c^\bu(\RR_{\geq 0})$ (and $\Tilde{\Omega}^\bu_c(\RR_{\geq 0})$) admits the obvious enhancement to a sheaf (cosheaf) of commutative dg algebras on $\RR_{\geq 0}$.
...
\[
\bC_n (\fg) (V \times [0,\epsilon)) = \clies\bigg(\fg \otimes \Omega^\bu(V) \Hat{\otimes} \Tilde{\Omega}^\bu([0,\epsilon))  \; , \; \Sym\left(\fg \otimes \Omega^\bu_c (V) \Hat{\otimes} \Tilde{\Omega}_{c}([0,\epsilon))[2] \right) \bigg).
\]
%\brian{this is wrong
%\[
...
\[
\begin{tikzcd}
\clies \bigg(\fg \otimes \Omega^\bu (U) \; , \; \Sym \left(\fg \otimes \Omega^\bu_c(U) [2] \right) \bigg) \; \otimes \;  \clies\bigg(\fg \otimes \Omega^\bu(V) \otimes \Tilde{\Omega}^\bu([0,\epsilon))  \; , \; \Sym\left(\fg \otimes \Omega^\bu_c (V) \Hat{\otimes} \Tilde{\Omega}_{c}([0,\epsilon))[2] \right) \bigg) \ar[d, "\simeq"] \\
\clies \bigg(\fg \otimes \left(\Omega^\bu (U) \oplus \Omega^\bu(V) \Hat{\otimes} \Tilde{\Omega}^\bu([0,\epsilon)) \right) \; , \; \Sym \left(\fg \otimes \left(\Omega^\bu_c(U) [2] \oplus \Omega^\bu_c(V) \Hat{\otimes} \Tilde{\Omega}_{c}^{\bu}([0,\epsilon)) [2]\right)\right) \bigg) \ar[d, "i^* \otimes i_*"] \\
\clies\bigg(\fg \otimes \Omega^\bu(V') \Hat{\otimes} \Tilde{\Omega}^\bu([0,\epsilon'))  \; , \; \Sym\left(\fg \otimes \Omega^\bu_c (V') \Hat{\otimes} \Tilde{\Omega}^{\bu}_{c}([0,\epsilon'))[2] \right) \bigg).
\end{tikzcd}
\]
...
\bigg(\fg \otimes \Omega^\bu_c(V) \Hat{\otimes} \Tilde{\Omega}^{\bu}_{c}([0,\epsilon)) [2]\bigg) \otimes \bigg(\fg \otimes \Omega^\bu(V) \Hat{\otimes} \Tilde{\Omega}^\bu([0,\epsilon)) \bigg) \ar[d,"{[\cdot, \cdot] \otimes \wedge}"] \\
...
\[
\bC_n(\fg)^\partial (V) = \clies\bigg(\fg \otimes \Omega^\bu(V) \Hat{\otimes} \Tilde{\Omega}^\bu(\RR_{\geq 0})  \; , \; \Sym\left(\fg \otimes \Omega^\bu_c (V) \Hat{\otimes} \Tilde{\Omega}_{c}^{\bu}(\RR_{\geq 0})[2] \right) \bigg) .
\]
On $V$, the map $I^\partial$ is defined by the composition
\[
\begin{tikzcd}
\clies\bigg(\fg \otimes \Omega^\bu(V) \Hat{\otimes} \Tilde{\Omega}^\bu(\RR_{\geq 0})  \; , \; \Sym\left(\fg \otimes \Omega^\bu_c (V \times \RR_{\geq 0})[2] \right) \bigg) \ar[r, "{{\rm ev}_1}"] & \Sym\left(\fg \otimes \Omega^\bu_c (V) \Hat{\otimes} \Tilde{\Omega}_{c}^{\bu}(\RR_{\geq 0})[2] \right) \ar[d, "{\int_{\RR_{\geq 0}}}"] \\ &  \Sym\left(\fg \times \Omega^\bu_c (V)[1] \right) .
\end{tikzcd}
\]
...
In the ordinary Batalin--Vilkovisky formalism on manifolds without boundary it is the central theme of \cite{CG2} that to any effective family of functionals satisfying the ``quantum master equation'' one can associate a factorization algebra of quantum observables.
On manifolds with boundary, this approach appears in the work of Rabinovich \cite{EugeneNew}.
