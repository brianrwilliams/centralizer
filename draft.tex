...
%We have developed locally constant factorization models separately for enveloping $\EE_n$-algebras and their centers.
...
The key insight is that there exists a filtration on our model for the $\EE_n$-center $\bZ_n(\fg)$ and the $\EE_n$-enveloping algebra $\bU_n (\fg)$ whose associated graded factorization algebras ${\rm Gr} \; \bZ_n(\fg)$ and ${\rm Gr} \; \bU_n(\fg)$ are commutative factorization algebras. 
...
Any constructible factorization algebra $\bA$ on $M \times \RR_{\geq 0}$ determines a factorization pair $(\pi_* \overset{\circ}{\bA} , \bA_{\partial})$ in the sense of Definition \ref{dfn:pair}.
...
\[
(\pi_* \overset{\circ}{\bf C}_n (\fg), {\bf C}_n (\fg)_\partial)
\]
...
\begin{dfn}
For an open set $U \subset M \times \RR_{>0}$ let $\overset{\circ}{\bC}_n(\fg) (U)$ be the cochain complex 
\[
\overset{\circ}{\bC}_n (\fg) (U) = \clie \bigg(\fg \otimes \Omega (U) \; , \; \Sym \left(\fg \otimes \Omega_c(U) [2] \right) \bigg) .
\]
\end{dfn}

The assignment $U \mapsto \overset{\circ}{\bC}_n(\fg)(U)$ defines a locally constant factorization algebra on $M \times \RR_{>0}$. 
In fact, it extends to a constructible factorization algebra on $M \times \RR_{\geq 0}$ in the following way. 

To state the definition, we will make use of the following map of cochain complexes. 
...
\begin{itemize}
\item to an open set $U \subset M \times \RR_{> 0}$, we assign the cochain complex $\overset{\circ}{\bC}_n (\fg)(U)$.
...
is
\[
\clie \bigg(\fg \otimes \Omega^\bu (U) \; , \; \Sym \left(\fg \otimes \Omega^\bu_c(U) [2] \right) \bigg) \; \otimes \;  {\rm Sym} \left(\fg \otimes \Omega^\bu_c (V) [1] \right) 
\]
\end{dfn}
...
Then, restricting to the boundary $M$, we obtain a locally constant factorization algebra that we will denote by $\bA_{\partial}$. 
Likewise, we can restrict to the bulk $M \times \RR_{> 0}$ and pushforward along $\pi : M \times \RR_{> 0} \to M$ to obtain another locally constant factorization algebra on $M$ that we will denote by $\pi_* \overset{\circ}{\bA}$. 
It is immediate to check that $(\pi_* \overset{\circ}{\bA} , \bA_{\partial})$ form a factorization pair in the sense of Definition \ref{dfn:pair}. 
