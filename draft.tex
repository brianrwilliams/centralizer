...
\end{equation}
%In particular, the cochain complex on the right-hand side has the structure of an $\EE_{n+1}$ algebra. 
...
%As a corollary of the above proposition, we obtain a concrete description for $\EE_n$-deformations of $\UU_n \fg$ when $\fg$ is an ordinary (strict) Lie algebra. 
...
%The augmentation $R_n = k \oplus k[1-n] \to k$,
%induces a canonical inclusion $i: \clies(\fg) \to \clies(\fg \otimes R_n)$ of dg associative algebras,
%and so
%\[
%k \otimes_{U\fg} U(\fg \otimes R_n) \simeq \Sym(\fg[1-n]).
%\]
...
%The following result follows from a simple application of the Poincar\'{e} lemma for de Rham forms and compactly supported de Rham forms.
...
In particular, ${\rm Gr} \; \UU_n (\fg)$ has the structure of a $\PP_n$-algebra.

There is a similar description of the ``classical limit"  of algebras over the $(n+1)$-dimensional Swiss cheese operad in terms of algebras over a certain two-colored operad $\PP_{[n+1,n]}$, introduced by Safronov \cite{Pavel1, Pavel2}.
Using our model, we will see that the pair $({\rm Gr} \; \fZ_n (\fg), {\rm Gr} \; \UU_n(\fg))$ is an algebra over this two-colored operad.
In particular, this implies that ${\rm Gr} \; \fZ_n (\fg)$ carries the structure of a $\PP_{n+1}$-algebra which ``acts'' on the $\PP_n$-algebra ${\rm Gr} \; \UU_n(\fg)$.
...
Following the proof of this proposition, our main thrust is to interpret this result by exhibiting a topological field theory on the $(n+1)$-dimensional manifold with boundary $M \times \RR_{\geq 0}$ whose factorization algebra of {\em classical} observables is equivalent to the constructible factorization algebra $\bC_n(\fg)$.
In this sense, the pair $(\bZ_n(\fg), \bU_n(\fg))$ is a model for the quantization of a particular topological field theory on a manifold with boundary.


\owen{Somewhere please mention that the slogan ``quantization commutes with pushforward'' holds for this class of theories.}
\brian{not sure where you wanted to address this request.}
...
For the remainder of this section we will assume that $\fg$ is a strict dg Lie algebra so that we can work with strict Poisson brackets
in the category of dg algebras.

\begin{dfn}
  A $\PP_m$-algebra is a commutative dg algebra $A$ equipped with a bracket
  \[
    \{\cdot , \cdot\} : A \otimes_k A \to A
  \]
  which defines a graded Lie bracket on $A[m-1]$ and satisfies the graded Leibniz rule
  \[
    \{a, b \cdot c\} = \{a,b\} \cdot c + (-1)^{|b||c|} \{a , c\} \cdot b .
  \]
\end{dfn}

A {\em $\PP_m$-factorization algebra} is a factorization algebra $\bA$ such that for every open set $U$ the cochain complex $\bA(U)$ is equipped with the structure of a $\PP_m$ algebra and that the factorization structure maps are compatible with the $\PP_m$-algebra structures.
In particular, we note that a $\PP_m$-factorization algebra is a commutative factorization algebra.

There is an additivity theorem of Dunn and Lurie for $\EE_n$-algebras, see Theorem ?? in \cite{LurieHA}, which states an equivalence of $\infty$-categories between the category of $\EE_{n+1}$-algebras and the category of algebras in $\EE_{n}$-algebras.
Safronov \cite{SafronovAdd} has proven a Poisson version of this result which states that there is an equivalence of $\infty$-categories between the category of $\PP_{n+1}$ algebras and the category of algebras in $\PP_{n}$-algebras.

In light of this result, the following lemma implies that ${\rm Gr} \; \bU_n (\fg)$ and ${\rm Gr} \; \bZ_n(\fg)$ determine $\PP_n$ and $\PP_{n+1}$ algebras respectively.
...
\begin{proof}
For any Lie algebra $\fh$, the associated graded of the Chevalley--Eilenberg complex ${\rm Gr} \; \cliels {\rm Sym} (\fh [1])$ is equipped with the structure of a $\PP_0$ algebra with $\PP_0$-bracket determined by the Lie bracket on $\fh$.
...
Roughly speaking, a {\em $\PP_{[n+1,n]}$ algebra} consists of a pair $(A,B)$ where $A$ is a $\PP_{n+1}$ algebra and $B$ is a $\PP_n$ algebra that receives an action by $A$.

The action of $\bZ_n(\fg)$ on $\bU_n(\fg)$ descends to an action at the level of the associated graded factorization algebras
Together with Lemma~\ref{lmm:pois1}, we see that the associated graded pair is an algebra over $\PP_{[n+1,n]}$.
...
Before stating the definition of this model, we introduce the following variants of the de Rham complex of differential forms on manifolds with boundary.

Suppose $N$ is a manifold with boundary and denote by $i_\partial : \partial N \hookrightarrow N$ the boundary embedding.
Let $\Omega^{\bu}(N \, | \, \partial N)$ be the subcomplex of de Rham forms $\alpha$ on $N$ with the property that $i_\partial^* \alpha  = 0$.
For instance, if $N = \RR_{\geq 0}$, then $\Omega^{\bu}(\RR_{\geq 0} \, | \, \{0\})$ is equal to the two-term complex
\[
  \{f \in  C^{\infty}(\RR_{\geq 0}) \; | \; f(0) = 0\}  \xto{\d_{\rm dR}} \Omega^1(\RR_{\geq 0})[-1]  .
\]
The complex $\Omega^\bu(N \, | \, \partial N)$ is a de Rham model for the relative cohomology of the pair $(N, \partial N)$.

Similarly, let $\Omega^{\bu}_{c}(N \, | \, \partial N)$ be the subcomplex of compactly supported de Rham forms $\alpha$ on $N$ with the property that $i_\partial^* \alpha = 0$.

Notice that both $\Omega^\bu(N \, | \, \partial N)$ and its compactly supported version are commutative dg subalgebras of the full de Rham complex.
...
\[
    \bC_n (\fg) (V \times [0,\epsilon)) = \clies\bigg(\fg \otimes \Omega^\bu(V \times [0,\epsilon) \, | \, V)  \; , \; \Sym\left(\fg \otimes \Omega^\bu_c (V \times [0,\epsilon) \, | \, V) [2] \right) \bigg)
\]
%\brian{this is wrong
%\[
...
\[
\begin{tikzcd}
\clies \bigg(\fg \otimes \Omega^\bu (U) \; , \; \Sym \left(\fg \otimes \Omega^\bu_c(U) [2] \right) \bigg) \; \otimes \;  \clies\bigg(\fg \otimes \Omega^\bu(V \times [0,\epsilon) \, | \, V) \; , \; \Sym\left(\fg \otimes \Omega^\bu_c (V \times [0,\epsilon) \, | \, V)[2] \right) \bigg) \ar[d, "\simeq"] \\
\clies \bigg(\fg \otimes \left(\Omega^\bu (U) \oplus \Omega^\bu(V \times [0,\epsilon) \, | \, V \right) \; , \; \Sym \left(\fg \otimes \left(\Omega^\bu_c(U) [2] \oplus \Omega^\bu_c(V \times [0,\epsilon) \, | \, V) [2]\right)\right) \bigg) \ar[d, "i^* \otimes i_*"] \\
\clies\bigg(\fg \otimes \Omega^\bu(V' \times [0,\epsilon') \, | \, V')  \; , \; \Sym\left(\fg \otimes \Omega^\bu_c (V' \times [0,\epsilon') \, | \, V') [2] \right) \bigg).
\end{tikzcd}
\]
...
\[
  \Omega^\bu (V \times [0,\epsilon) \, | \, V) \xto{j^*}  \Omega^\bu (V \times (0,\epsilon)) \xto{\iota^*} \Omega^\bu(U)
\]
where $j : V \times (0,\epsilon) \hookrightarrow V \times [0,\epsilon)$ is the open embedding.
...
\[
{\rm Hom}^\bu\left(\Sym(\fg \otimes \Omega^\bu (V \times [0,\epsilon) \, | \, V) [1]) \; , \; \Sym \left(\fg \otimes \Omega^\bu_c(V \times [0,\epsilon) \, | \, V) [2] \right) \right)  .
\]
We first define the bracket $\{F,G\}$ between the following simple elements 
\[
F : \fg \otimes \Omega^\bu(V \times [0,\epsilon) \, | \, V) [2] \to k
\]
and 
\[
G : k \to \fg \otimes \Omega^\bu_c(V \times [0,\epsilon) \, | \, V)[2] .
\]
Define $\{F, G\}$ by the composition
\[
\begin{tikzcd}
k \otimes \bigg(\fg \otimes \Omega^\bu(V \times [0,\epsilon) \, | \, V) \bigg) \ar[rdd, dotted, "{\{F,G\}}"'] \ar[r, "G \otimes 1"] &
\bigg(\fg \otimes \Omega^\bu_c(V \times [0,\epsilon) \, | \, V) [2]\bigg) \otimes \bigg(\fg \otimes \Omega^\bu(V \times [0,\epsilon) \, | \, V)\bigg)
\ar[d,"{[\cdot, \cdot] \otimes \wedge}"] \\
& \fg \otimes \Omega^\bu(V \times [0,\epsilon) \, | \, V) [2] \ar[d, "F"] \\ & k
\end{tikzcd}
\]
...
\[
  \bC_n(\fg)^\partial (V) = \clies\bigg(\fg \otimes \Omega^\bu(V \times \RR_{\geq 0} \, | \, V) \; , \; \Sym\left(\fg \otimes \Omega^\bu_c (V \times \RR_{\geq 0} \, | \, V)[2] \right) \bigg) .
\]
On $V$, the map $I^\partial$ is defined by the composition
\[
\begin{tikzcd}
  \clies\bigg((\fg \otimes \Omega^\bu(V \times \RR_{\geq 0} \, | \, V) \; , \; \Sym\left(\fg \otimes \Omega^\bu_c (V \times \RR_{\geq 0})[2] \right) \bigg) \ar[r, "{{\rm ev}_1}"] & \Sym\left(\fg \otimes \Omega^\bu_c (V \times \RR_{\geq 0} \, | V)[2] \right) \ar[d, "{\int_{\RR_{\geq 0}}}"] \\ &  \Sym\left(\fg \otimes \Omega^\bu_c (V)[1] \right) .
\end{tikzcd}
\]
The first map ${\rm ev}_1$ evaluates the unit in $\Sym \left(\fg \otimes \Omega^\bu(V \times \RR_{\geq 0} \, | \, V)[1]\right)$ and the second map is induced by integration along $\RR_{\geq 0}$.
...
The value of the factorization algebra on an open set $U$ is, by definition, the functionals on the fields restricted to $U$.
Furthermore, the observables are equipped with the structure of a $\PP_0$ factorization algebra.
...
\[
    \cL (V \times [0,\epsilon)) = \Omega^\bu (V \times [0,\epsilon) \, | \, V) \otimes \fg \ltimes \Omega^{\bu} (V \times [0,\epsilon)) \otimes \fg^* [n-3] .
\]
\end{itemize}
\end{dfn}
...
\begin{proof}
  This proposition follows by tracing through some definitions, together with a key fact in functional analysis.

  First, observe that the cochain complex which is {\em linear dual}\footnote{Here, we mean continuous linear dual, taking into account the structure of de Rham forms as a topological vector space.} to the cochain complex $\Omega^\bu (V \times [0,\epsilon)) \otimes \fg^*[n-3]$ is
  \[
    \Bar{\Omega}_c^\bu(V \times [0,\epsilon) \, | \, V) \otimes \fg[2].
  \]
  Here, the bar in $\Bar{\Omega}_c$ indicates compactly supported distributional differential forms.
  In words, this is the space of compactly supported distributional de Rham forms $\alpha$ valued in $\fg[2]$ with the property that $i_\partial^* \alpha = 0$.
  Where, as above, $i_\partial : \partial N \hookrightarrow N$ is the boundary embedding.

  Now, there is a natural inclusion of cochain complexes of the smooth (non-distributional) compactly supported de Rham forms inside of the distributional ones:
  \begin{equation}\label{eqn:AB}
    \Omega_c^\bu(V \times [0,\epsilon) \, | \, V) \otimes \fg[2] \hookrightarrow \Bar{\Omega}_c^\bu(V \times [0,\epsilon) \, | \, V) \otimes \fg[2] .
  \end{equation}
  Furthermore, using ellipticity of the de Rham operator we can assert by the Atiyah--Bott lemma \cite{??} that this inclusion is a quasi-isomorphism.

  With this bit of functional analysis out of the way, we observe that the value of the factorization algebra $\clies(\cL)$ on $V \times [0,\epsilon)$ is


  The value of $\clies(\cL)$ on an open set $U$ is
  \[
    \clies\bigg(\fg \otimes \Omega^\bu(V \times [0,\epsilon) \, | \, V)  \; , \; \Sym\left(\fg \otimes \Bar{\Omega}^\bu_c (V \times [0,\epsilon) \, | \, V) [2] \right) \bigg) .
\]
So, the quasi-isomorphism (\ref{eqn:AB}) induces a quasi-isomorphism
\[
  \bC_n(\fg)(V \times [0,\epsilon)) \xto{\simeq} \clies(\cL)(V \times [0,\epsilon)) .
\]

Checking that $\bC_n(\fg)(U) \to \clies(\cL)(U)$ for open sets of the form $U \subset M \times \RR_{>0}$ is completely similar.
...
By Proposition \ref{prp:pair}, the constructible factorization algebra $\bC_n(\fg)$ is the classical limit of the factorization sc-pair $(\bZ_n(\fg), \bU_n(\fg))$.
Thus, we have shown that the pair $(\bZ_n(\fg), \bU_n(\fg))$ is
a model for the quantum factorization algebra of observables of topological BF theory on $M \times \RR_{\geq 0}$.


In the ordinary Batalin--Vilkovisky formalism, on manifolds without boundary, it is the central result of \cite{CG2} that to any effective family of functionals satisfying the ``quantum master equation'' one can associate a factorization algebra of quantum observables.
For manifolds with boundary, this has been developed in the work of Rabinovich \cite{EugeneNew}.
We expect that one can exhibit our constructible factorization algebra as the quantum factorization algebra of observables associated to a Batalin--Vilkovisky quantization of BF theory on $M \times \RR_{\geq 0}$.
...
\owen{revisit and revise}

%By definition~\ref{dfn:constructible1}, we have a model for the pair $(\Gr \frak{Z}_n(\fg), \Gr \UU_n \fg)$.
%To see this, observe that to an open subset $V$ of the interior $\mathring{M}$, 
%the factorization algebra assigns
%\begin{equation}
%\label{eqn: gr Z}
%\clies(\Omega^*(V) \otimes \fg, \Sym(\Omega^*_c(V) \otimes \fg[2])).
%\end{equation}
%Given an open set $U \subset \partial M$ and a small open neighborhood in $M$ of $U$ such as $U \times [0,\epsilon)$ (depending on a choice of local collar coordinate), 
%the factorization algebra assigns
%\[
%\Sym(\Omega^*_c(U)\otimes \fg[1]),
%\]
%up to quasi-isomorphism.
%
%What we really want is a convenient model for this entire factorization algebra whose behavior recovers with the values above.
%Thankfully, it is straightforward to find one, via classic algebraic topology.
%For any open subset $V$ in $M$, define
%\[
%\Omega^*_{{\rm rel }}(V) = \{ \alpha \in \Omega^*(V) \, : \, \alpha|_{\partial M} = 0\}.
...
%F_{{\rm rel}}(V) =\clies(\Omega^*_{{\rm rel }}(V) \otimes \fg, \Sym(\Omega^*_c(V) \otimes \fg[2])).
%\]
%When $V$ is an open set inside $\mathring{M}$, we find $F_{{\rm rel}}(\mathring{M})$ recovers \eqref{eqn: gr Z} from above.
%When $V$ is a product open $U \times [0,\epsilon)$ along the boundary, we find 
%\[
%F_{{\rm rel}}(V) \simeq \clies(0, \Sym(\Omega^*_c(V) \otimes \fg)) \simeq \Sym(\Omega^*_c(V) \otimes \fg),
%\]
%as desired.
