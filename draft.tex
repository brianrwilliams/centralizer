...
\owen{It might be better to first use the adjunction to get a map of homotopy-coherent Lie algebras $\fg \to B$ but I'm not sure this plays nicely with the other strictification\dots}
\item From the map ${\tt f}$, we read off an $\fg$-action on ${\tt B}$ intertwining with the structure maps of ${\tt B}$. 
That is, we have a prefactorization algebra ${\tt B}_f$ taking values in the 1-category $\Ch(\Rep(\fg))$ of chain complexes of $\fg$-representations.
