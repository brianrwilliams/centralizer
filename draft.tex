...
\owen{We could describe more that we do.}
...
For a reductive Lie algebra $\fg$, the Harish-Chandra isomorphism is an isomorphism of commutative algebras
\[
Z(U(\fg)) \xto{\cong} {\rm Sym} (\fh)^W
\]
where $\fh$ is a Cartan subalgebra and $W$ is the Weyl group of $\fg$. 
Here $Z(U(\fg))$ denotes the ordinary center of the associative algebra $U(\fg)$. 

The following result generalizes (and implies) the Harish--Chandra isomorphism for the $\EE_n$-center of the $\EE_n$-enveloping algebra. 
...
\[
{\frak Z}_n(\UU_n \fg) \simeq H^\bu_{\rm Lie}(\fg) \otimes \Sym(\fh[1-n])^W.
\]
\end{prp}
...
\[
\clies(\fg, \Sym(\fg[1-n]) \simeq H^\bu_{\rm Lie} (\fg) \otimes \Sym(\fg[1-n])^G 
\]
...
\owen{Please check this carefully, Greg. The lemma is meaningful even if the proposition is bogus.}
...
These can be applied to our models of the center to obtain novel $\EE_n$ algebras.
...
\begin{prp}
There is a quasi-isomorphism of $\EE_n$ algebras
\[
{\frak Z}_n(\frak{gl}_\infty) \xto{\simeq} \clies(\frak{\gl}_\infty) \otimes \Sym(\Cyc_*(k[-n])[1])  ,
\]
and so, in particular, the $N \to \infty$ limit of ${\frak Z}_n(\frak{gl}_N)$ is commutative.
\end{prp}

\begin{proof}
The Loday-Quillen-Tsygan (LQT) theorem --- for nonunital algebras --- provides a quasi-isomorphism
\[
\cliels(\gl_\infty(A))^{\gl_\infty(k)} \xto{\simeq} \Sym(\Cyc_*(A)[1])
\] 
and it is functorial in the algebra~$A$.
\owen{I used {\em invariants} here rather than {\em coinvariants}. Is that allowed?}
Now
\[
\clies(\gl_\infty, \cliels(\gl_\infty(A)) \simeq \cliels(\gl_\infty) \otimes \cliels(\gl_\infty(A))^{\gl_\infty(k)},
\]
as shown in the proof of the LQT theorem.

We take $A = \Omega^*_c(V)$ and use the functoriality to see that we get a quasi-isomorphism of prefactorization algebras, as claimed.
\end{proof}

There are immediate analogs of this theorem where we replace $\gl_N$ by $\frak{so}_N$ or $\frak{sp}_N$ and where we replace cyclic chains by dihedral chains.

A striking consequence of the LQT map appears at the level of factorization homology.

\begin{lmm}
For a closed $n$-dimensional manifold~$M$, 
\begin{align*}
\int_M {\frak Z}_n(\frak{gl}_\infty) 
&\simeq \clies(\Omega^*(M) \otimes \gl_\infty, \Sym(\Cyc_*(\Omega^*(M))[1])) \\
&\simeq \clies(\Omega^*(M) \otimes \gl_\infty, \Sym(\Omega^*(LM/^h S^1))[1]))
\end{align*}
where $\Omega^*(LM/^h S^1)$ denotes some cochain model for the cochains of the homotopy quotient of the free loop space under the canonical rotation action on loops.
\end{lmm}

\owen{That phrasing could be improved.}

Implicit in the statement is that one must describe the needed (and non-obvious) action of $\gl_\infty(\Omega^*(M))$ on a symmetric algebra generated by cyclic chains, respectively the rotation-equivariant cochains on the free loop space~$LM$.

What makes this result striking is that it shows how the {\em cyclic loop space} $LM/^h S^1$ appears out of taking a large $N$ limit of factorization homology.
...
In Section~\ref{sec: TFT} we explain how the center $\frak{Z}_n(\gl_\infty)$ can be understood as the {\em quantum observables} of topological BF theory for the Lie algebra $\gl_\infty$.
In this sense, the left hand side of the lemma $\int_M {\frak Z}_n(\frak{gl}_\infty)$ describes the global quantum observables over the manifold $M$.
...
\begin{proof}
As $\Omega^*_c(M) = \Omega^*(M)$ because $M$ is closed, 
one applies the LQT theorem for {\em unital} algebras
\[
\cliels(\gl_\infty(\Omega^*(M))) \xto{\simeq} \Sym(\Cyc_*(\Omega^*(M))[1])
\]
to obtain the first weak equivalence.
(In fact, one must transfer the $\gl_\infty(\Omega^*(M))$ along the LQT map.)
Then Jones' theorem \cite{} provides the second weak equivalence.
\end{proof}
...
\begin{proof}
Our model $\bZ_n(\fg)$ is an strict associative algebra, and our models $\clies(\Omega^* \otimes \fg)$ and $\bU_n\fg$ are strict right, respectively left, modules over $\bZ_n(\fg)$.
On each open set~$V$, we compute
\begin{align*}
\clies(\Omega^*(V) \otimes \fg) &\otimes^\LL_{\bZ_n(\fg)(V)} \bU_n(V) \\
&\simeq \clies(\Omega^*(V) \otimes \fg) \otimes^\LL_{\Hom^\bullet(\cliels(\Omega^*(V) \otimes \fg, \cliels(\Omega^*_c(V)\otimes \fg))} \cliels(\Omega^*_c(V)\otimes \fg)
\end{align*}
...
\[
\begin{tikzcd}
\clies \bigg(\fg \otimes \Omega^\bu (U) \; , \; \Sym \left(\fg \otimes \Omega^\bu_c(U) [2] \right) \bigg) \; \otimes \;  {\rm Sym} \left(\fg \otimes \Omega^\bu_c (V\times [0,\epsilon)) [2] \right) \ar[d, "{\rm ev}_1"] \\
...
\end{tikzcd}
\]
The first map ${\rm ev}_1$ evaluates the unit $1$ in $\Sym\left(\fg \otimes \Omega^\bu(U)\right)$ and the second map uses the fact that the symmetric algebra functor is monoidal. 
...
\]
Let $x^* \otimes \alpha$ denote an homorphism of the form $\fg \otimes \Omega (U) \to \Sym^0 = k$ and $y \otimes \beta$ denote a homomorphism $k = \Sym^0 \to \fg \otimes \Omega_c(U) [2]$. 
...
The pair $(\int_{\RR_{>0}} , \int_{\RR_{\geq 0}})$ is a quasi-isomorphisms of factorization sc-pairs. 
...
As a field theory, there is a system of PDE known as the ``solutions to the equations of motion.''
When $N$ is compact, the moduli space of solutions to these equations of motion for topological BF theory can be described as the mapping space 
\begin{equation} 
\label{stack1}
...
%
%Let us place the locally constant factorization algebra model of the $\EE_n$-enveloping algebra $\bU_n (\fg) = \clieu(\fg \otimes \Omega_c)$ on the $n$-manifold $M$.
...
%Locally, on disks, we will see that it is quasi-isomorphic to a certain limit, or ``dequantization", of our model for the $\EE_n$-center of $\UU_n \fg$.
...
%\]
%Notice this is similar to the model $\EE_n$-center except we have turned off the Chevalley--Eilenberg differential on chains. 
%
% \begin{lmm}
