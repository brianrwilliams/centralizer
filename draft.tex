...
\end{tikzcd}
\]
where $u$ is the tensor product of the unit map $1_\cC \to {\frak Z}_{E_n}(f)$ and the identity map $A \to A$.
...
Once that is in place, it is straightforward to describe the structure maps of the prefactorization algebra.

The role of 

The lemma allows us to define the factorization algebra given on any open $U$ by  the Chevalley-Eilenberg chain complexes
$ \cliels\big(\fg \otimes \Omega^*_c(U), \bU_n \fh(U) \big)$ (with their standard differential)
of $\fg \otimes \Omega^*_c(U)$ with value on $\bU_n \fh(U)$ whose structure maps are given, for any $(U_i)_{i\in I}$ 
a finite family of pairwise disjoint open subsets of an open $V$, by 
\begin{multline}\label{eq:transClieFac}
 \bigotimes_{i\in I} \cliels\big(\fg \otimes \Omega^*_c(U_i), \bU_n \fh(U_i) \big) \cong Sym\Big(
...
\label{L:ClieFacDef} 
 ...
 $ \clies\big(\fg \otimes \Omega^*, \bU_n \fh \big)$ a (resp. locally constant) factorization algebra.

\begin{dfn}\label{dfn:CEofFact}
We {call   $\cliels\big(\fg \otimes \Omega^*_c, \bU_n \fh \big)$ the Chevalley-Eilenberg chain factorization algebra of $\fg$ with value in $\bU_n \fh$} 
and $\clies\big(\fg \otimes \Omega^*, \bU_n \fh \big)$ the Chevalley-Eilenberg cochain factorization algebra.
\end{dfn}
\begin{proof}
...
A special but very interesting case is of course when $\bU_n \fh$ is itself an enveloping algebra. 
\begin{cor}
