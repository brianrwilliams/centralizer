...
Recently, a generalization of the concept of the enveloping algebra of a Lie algebra has emerged: each dg Lie algebra $\fg$ has an $E_n$-enveloping algebra $\UU_n\fg$, for every $n$, so that $\UU_1\fg$ is the usual enveloping algebra $U\fg$. Building on prior work \cite{BD, AF} [Owen's thesis?], Knudsen \cite{Knudsen} developed these ideas extensively and offered a useful factorization algebra model for the $E_n$ enveloping algebra. Our central goal here is to generalize the following results about the enveloping algebra to all $n$.
...
\begin{rmk}
We state our theorems in terms of the stable $\infty$-category cochain complexes instead of the appropriate stable model or pre-triangulated dg categories. Many of our constructions, however, involve explicit manipulations of standard complexes. We indicate the level at which we are working, when it is helpful.
\end{rmk}
...
Let $\UU_n \fg$ is the enveloping $E_n$ algebra of a dg Lie algebra $\fg$ and $f: \UU_n \fg \to B$ a map of $E_n$ algebras, which gives $B$ the structure of an $E_n$ algebra in $\fg$-modules. Then, the centralizer ${\frak Z}_{E_n}(f)$ is the $E_n$ algebra $\clies(\fg, B)$.
\end{thm}
...
\begin{cor}
Let $k$ is $\RR$ or $\CC$, and $\UU_n \rho:  \UU_n \fg \to \UU_n \fh$ be the induced by a map of Lie algebras $\rho: \fg \to \fh$.  Then, the centralizer ${\frak Z}_{E_n}(\UU_n \rho)$ is modeled by the locally constant factorization algebra that assigns
\[
\clies(\fg \otimes \Omega^*(U), \cliels( \fh \otimes \Omega^*_c(U)))
\]
to each open set $U \subset \RR^n$, where $\Omega^*_c(U)$ denoted the commutative differential graded algebra (cdga) of compactly supported forms on $U$.
\end{cor}

When $\rho: \fg \to \fg$ is the identity map, we have the following immediate corollary.

\begin{cor}
\label{thm: center}
The center of the enveloping $E_n$ algebra $\UU_n\fg$ is the $E_n$ algebra $\clies(\fg, \UU_n\fg)$, which by Dunn's additivity theorem has in fact a natural $E_{n+1}$ algebra structure. When $k$ is $\RR$ or $\CC$, the 
...
\end{cor}
\greg{shall relate this to $P_n$-center of $\fg$. that shall also (but I have to think to the proof more), at least up to formality gives the $P_{n+1}$-structure on it. }
...
Having an explicit presentation for the centralizer of a map of $E_n$ algebras leads to a calculations of factorization homology. The enveloping $E_n$ algebras are homotopy $O(n)$-fixed points, or a $\mathrm{Disk}_n$-algebras in the sense of~\cite{AF}, and hence determine factorization algebras on un-oriented $n$-dimensional manifolds \cite{Knudsen}. Therefore, for a closed $n$-dimensional manifold $M$, one has
\[
...
Our novel contribution in this paper is the following large $N$ result. Let $A$ be a unital algebra over a field $k$ of characteristic zero, and $u_A: \sl_\infty(k) \hookrightarrow \gl_\infty(A)$, the map of Lie algebras induced by the unit in $A$. The induced map $\UU_nu_A: \UU_n\sl_\infty(k) \to \UU_n\gl_\infty(A)$ makes $\UU_n\gl_\infty(A)$ is an $E_n$ algebra in $\sl_\infty(k)$-modules.
...
Again, the explicit factorization algebras provide convenient methods for computing factorization homology.
...
Although \cite{Knudsen} develops an adjunction between Lie and $E_n$ algebras in any presentable symmetric monoidal stable $\infty$-category $\cC$,
we need his result only for that of chain complexes $(\Mod_k, \otimes_k)$, where $k$ is a field of characteristic zero. We summarize Knudsen's result in this setting as follows.
...
\end{tikzcd}
\]
where $\Sigma$ denotes suspension \owen{because tikzcd does weird things with ``[1-n]'' instead}, \mahmoud{earlier we denoted the shift by [n]. should we change that?}.
...
\end{dfn}
When $n=1$ and $\cC$ is the category of vector spaces, the centralizer of $f: A \to B$ thus consists of the subspace of elements of $B$ that commute with the image $f(A)$ inside~$B$. Thus, the center of $A$ agrees with the traditional notion. See Theorem 5.3.1.30 of~\cite{LurieHA}, or Proposition 6.22 in~\cite{GTZ3}, for an alternative characterization.
...
\]
Recall now that $\UU_n \fg \simeq \Sym(\fg[1-n])$ as $\fg$-modules \greg{shall we also need that it is true as $E_n$-algebra one we see right hand side as a $P_n$-algebra in the natrual way and use formality ? I feel we need to say something more to recover the $E_n$-structure below-something like the $E_n$-structure on Lie algebra cochains realizes teh moduel map structure after all equivalences we use}.
...
Throughout this section, let $A$ be a dg algebra over a field $k$ of characteristic zero. We do not assume $A$ is unital, unless we state otherwise. Also, it is possible to work with $A_\infty$ algebras rather than dg algebras, but it changes nothing structurally essential while it adds a distracting layer of complexity. Let $\cL(A)$ denote the dg Lie algebra $\gl_\infty(A)$ and $\cC(A)$ denote the cochain complex $\Cyc_*(A)[1]$ thought of as an abelian differential graded algebra. Let $\gl_\infty(A)$ denotes the infinite matrices with only finitely many nonzero entries in~$A$. Let $\gl_\infty$ stand for $\gl_\infty(k)$ and $\sl_\infty=\sl_\infty(k)$ denote the sub Lie algebra of the elements of trace is zero. There is a natural action of $\sl_\infty$ on $\gl_\infty(A)$, and hence on its Chevalley-Eilenberg chains $\cliels(\cL(A))$. A key step in the LQT theorem is the construction of an isomorphism
\beqn\label{lqtco}
