...
\end{thm}
\greg{shall explain/remind somewhere what is the $E_n$-structure there: namely induced by $E_n$-structure of $B$ and com mutliplication in $\clies$, namely convolution between an $E_\infty$-coalgebra and an $E_n$-algebra}
...
Indeed, as shown  in \cite{Knudsen}, 
the enveloping $E_n$ algebras are homotopy $O(n)$-fixed points (in other words $\mathrm{Disk}_n$-algebras in the sense of~\cite{AF}) and hence determine factorization algebras on unoriented $n$-dimensional manifolds.

Therefore, for $M$ a closed $n$-dimensional manifold, one has 
\[
...
An alternative characterization is found in Theorem 5.3.1.30 of~\cite{LurieHA} or Proposition 6.22 in~\cite{GTZ3}.
...
\end{cor}
\greg{why not state (in a remark) this corollary and the next one as syaing this is an equivalence in the $\infty$-categories of functor ?}
